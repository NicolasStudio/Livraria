...
<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submitExcluir'])) {
        ...
        // Consulta para obter o caminho da imagem associada ao produto
        $query_imagem = "SELECT img FROM produtos WHERE nome = '$titulo'";
        $resultado_imagem = mysqli_query($conexao, $query_imagem);

        if ($resultado_imagem && mysqli_num_rows($resultado_imagem) > 0) {
            $row = mysqli_fetch_assoc($resultado_imagem);
            $caminho_imagem = 'Img/Livros/' . $row['img'];
        ...
        $query = "DELETE FROM produtos WHERE  nome = '$titulo'";
...
<body>
    ...
    <div class="boxProdutos">
        <h6 class="titleExcluir">Excluir Produtos</h6>
        ...
        <form action="excluirProduto.php" method="POST" enctype="multipart/form-data">
        ...
        <form action="excluirProduto.php" method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" name="submitExcluir" class="excluirCad">Excluir Produto</button>
